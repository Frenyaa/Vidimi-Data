...
excel_path = '/Users/buidoanhaiyen/Documents/Vidimi/Code/6. Data/VN_FnB_Master_Expansion_v21_Lays_HuuNghi_Cholimex_Vissan_Mentos_2026.xlsx'
keywords_path = '../config/keywords.json'

print("Reading v21 Master Expansion file...")
...
def map_category(eng_cat):
    ...
    if any(x in eng_cat for x in ['milk tea', 'fruit tea', 'tea latte', 'brewed tea', 'pure tea', 'tea', 'matcha']):
        return 'BEV_TRA_SUA' 
    if any(x in eng_cat for x in ['coffee', 'espresso', 'cà phê', 'latte', 'macchiato']):
        return 'BEV_CAFE'
    if any(x in eng_cat for x in ['instant', 'noodle', 'mì', 'pho', 'bún']):
        return 'FOOD_AN_SAN'
    if any(x in eng_cat for x in ['snack', 'candy', 'gum', 'dessert', 'ice cream', 'biscuit', 'cake', 'bakery', 'buns', 'toasts', 'bread', 'confectionery', 'cookie', 'cracker', 'crisp', 'bánh trung thu']):
        return 'FOOD_SNACK' 
    ...
    if any(x in eng_cat for x in ['sauce', 'seasoning', 'topping']):
        return 'FOOD_OTHER'
    if any(x in eng_cat for x in ['sausage', 'giò', 'meat', 'canned food', 'frozen food', 'chicken', 'pork', 'beef', 'fish']):
        return 'FOOD_THIT_CA'
    if any(x in eng_cat for x in ['salad', 'fruit', 'vegetable']):
        return 'FOOD_RAU_CU_QUA'
    if any(x in eng_cat for x in ['combo']):
        return 'COMBO/SET'
    if any(x in eng_cat for x in ['pizza', 'pasta', 'burger']):
        return 'FOOD_LPN' 
    return None
...
for idx, row in df.iterrows():
    ...
    if target_cat:
        ...
        if pd.notna(brand_product_key):
            # Convert "burger_king::burger_2_mieng..." into "burger king burger 2 mieng..."
            clean_key = str(brand_product_key).replace("::", " ").replace("_", " ")
            add_to_dict(target_cat, clean_key)
            added_count += 1 # Only counting keys from this branch for tracking

with open(keywords_path, 'w', encoding='utf-8') as f:
    json.dump(keywords, f, ensure_ascii=False, indent=2)

print(f"Successfully added many new normalized and brand keys from Master Expansion v21!")
